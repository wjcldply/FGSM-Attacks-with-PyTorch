# CUDA_VISIBLE_DEVICES=1 nohup python -u project_8.py --port 6800 > project_8_testResults.out 2>&1 &
setup = "[ NO Normalization | step: alpha=epsilon/iter | type: Nestrov_Accelerated-MI-FGSM ]"
    ...
    for epsilon in epsilons:
        ...
        for k in range(epochs):  # 에포크 만큼
            ...
            for _ in range(max_iteration):  # I-FGSM
                # Calculate "Nestrov Gradient Component"
                nestrov = alpha * decay * delta
                X_nestrov = X_ori + delta + nestrov

                logits = resnet(norm(X_nestrov))  # 노이즈 추가한 이미지를 ResNet이 분류한 분산표상
